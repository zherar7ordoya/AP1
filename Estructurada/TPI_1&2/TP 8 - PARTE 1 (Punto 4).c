...
void mostrar_struct(struct empleado empleados[], int registros, char modo);
...
void calcular_semestrales(struct empleado empleados[], struct empleado semestrales[]);
void maximo_semestral(struct empleado semestrales[], int registros);
void mostrar_superadores(long matriz[][2]);
void ordenar_struct(struct empleado listado[], int registros);
////////////////////////////////////////////////////////////////////// PRINCIPAL

    /*
	Una  empresa  desea  analizar  los  sueldos  a  sus 50  empleados  durante
	el  año,  para  ello conoce:                               
		a.  Legajo del empleado                                
...
int main()
{
    ...
    // PUNTO E
    printf("\n\ne) nombre del empleado que cobró más en el primer semestre:\n\n");
    struct empleado semestrales[activos];
    calcular_semestrales(empleados, semestrales);
    maximo_semestral(semestrales, activos);

    // PUNTO F
    printf("\n\nf) cantidad de meses en que el total de sueldos superó los $ 100.000:\n\n");
    long renovar_mensuales[12][2];
    calcular_mensuales(empleados, renovar_mensuales);
    mostrar_superadores(renovar_mensuales);

    // PUNTO G
    printf("\n\ng) imprimir ordenado en forma descendente por sueldo anual de cada empleado:\n\n");
    ordenar_struct(anuales, activos);
    mostrar_struct(anuales, activos, ABREVIADO);

    // FINALE
    char respuesta;
    printf("\n\n ¿Desea listar la carga de empleados a fin de corroborar los resultados?");
    printf("  \n ****************** ADVERTENCIA: EL LISTADO ES LARGO! ****************** \n\n");

    do
    {
        printf("\t\t\t  ¿Proceder? (Sí/No): ");
        scanf("%c", &respuesta);
        getchar();
    ...
    {
        mostrar_struct(empleados, 600, COMPLETO);
    }
    else
    {
        printf("\n\t\t\t          ~FIN~\n");
...
{
    int loopex, loopin;
    struct empleado temporal;

    for (loopex = 0; loopex < registros - 1; loopex++)
    {
        for (loopin = 0; loopin < (registros - 1 - loopex); loopin++)
        {
            if (listado[loopin].sueldo < listado[loopin + 1].sueldo)
            {
                temporal = listado[loopin];
                listado[loopin] = listado[loopin + 1];
                listado[loopin + 1] = temporal;
            }
        }
    }
}
...
void mostrar_superadores(long matriz[][2])
{
    ...
    {
        if (matriz[loop][1] > 100000)
        {
            printf(" %2d \t $ %6d \n", (int)matriz[loop][0], matriz[loop][1]);
        }
    }
}
...
void maximo_semestral(struct empleado semestrales[], int registros)
{
    ...
    {
        if (semestrales[iter].sueldo > maximo)
        {
            legajo = semestrales[iter].legajo;
            maximo = semestrales[iter].sueldo;
        }
    }

    printf(" LEG \tNOMBRE Y APELLIDO \t*TOTAL* \n\n");
    printf(" %3d \t%s \t$ %5d \n", legajo, bautizame(legajo), maximo);
}
...
void calcular_semestrales(struct empleado empleados[], struct empleado semestrales[])
{
    ...
    {
        if (empleados[ite].mes < 7)
        {
            if (ite == 0)
            {
                semestrales[aux] = empleados[ite];
            }
            else
            {
                for (ita = 0; ita <= aux; ita++)
                {
                    if (empleados[ite].legajo == semestrales[ita].legajo)
                    {
                        semestrales[ita].sueldo += empleados[ite].sueldo;
                        break;
                    }
                    if (ita == aux)
                    {
...
void mostrar_struct(struct empleado empleados[], int registros, char modo)
{
    ...
    if (modo == 'A')
    {
        ...
        printf("LEG \tNOMBRE Y APELLIDO \t\tSUELDO\n\n");
        for (loop = 0; loop < registros; loop++)
        {
            printf("%3d  \t%s  \t$ %4d \n",
                   (int)empleados[loop].legajo,
