 ...
 * Comisión 1-ON
 * 
 * @brief TRABAJO PRÁCTICO OBLIGATORIO 2 (PUNTO 2)
...
void romanoadecimal(char romano[], int decimal);
...
int main()
{
	...
	int bucle = 0, escape = 0, resultado = 0, contador = 0;
	char terminar = 'S';
	...
	while (terminar != 'N' && terminar != 'n')
	{
		...
		} while (letras[0] == '\n' || strlen(letras) > 11);	// Limitación pedida.
		...
		else
		{
			...
			romanoadecimal(letras, resultado);	// Nombre de función pedida.
...
void romanoadecimal(char romano[], int decimal)
{
	printf("\n\tEl decimal equivalente de %s es: \t%d\n\n", romano, decimal);
}
